# -*- coding: utf-8 -*-
"""
Created on Thu Jun 27 13:56:24 2019
...
# -*- coding: utf-8 -*-
"""
Created on Wed Jun 26 21:30:07 2019
...
S_Wu, S_Wp = scatter_within(X, y_utility), scatter_within(X, y_privacy)
S = S_Bu + S_Wu + Regularization

# Giving more weight to y-privacy for RUCA matrix
P=1000
S_RUCA = S + P*S_Bp
# getting the eigen value and eigen vectors for W matrix
eigen_vals, eigen_vecs = np.linalg.eigh(np.linalg.inv(S_RUCA).dot(S_Bu_reg))
...
# DCA Transformation from 1 to 15 features
Accuracy_U_RUCA = []
Accuracy_P_RUCA = []
for i in range (1, 15):    
...
#UTILITY TARGET CLASSIFICATION SVM USING DIFFERENT KERNELS WITH 5 FOLD CROSS-VALIDATION USING DEFFERENT GAMMAS AND C's 
    ...
    Accuracy_U_RUCA.append(accuracy_utility)
...
#PRIVACY TARGET CLASIFICATION SVM USING DIFFERENT KERNELS WITH 5 FOLD CROSS-VALIDATION USING DEFFERENT GAMMAS AND C's 
    ...
    Accuracy_P_RUCA.append(accuracy_privacy)